...
'''
@Project ：Paper_模拟网络 
@File    ：Fig-DD-WD(可视化).py
@IDE     ：PyCharm 
@Author  ：Vinne
@Date    ：2025/6/24 12:49 
@Explain ：
'''
import re
import numpy as np
...
# 读取保存的 CSV
df_all = pd.read_csv("data/result/RLvsReal_WD.csv")
# 提取所有真实-实验网络配对
network_pairs = set()
...
# 创建一个映射：网络名称 -> 最优EMD
emd_map = dict(zip(real_net_info['网络名称'].astype(str), real_net_info['RL(WD)']))
...
for i, real_name in enumerate(available_pairs):
    ...
    emd_text = f"WD: {emd_value:.4f}" if emd_value is not None else "WD: N/A"
...
plt.savefig("D:\科研论文文稿\Paper_RL\图\Figs\Fig-DD-WD.pdf", format="pdf")
plt.savefig('D:\科研论文文稿\Paper_RL\图\Figs\Fig-DD-WD.svg', format='svg')
# plt.show()
