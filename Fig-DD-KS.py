...
# 读取保存的 CSV
df_all = pd.read_csv("data/result/RLvsReal_KS.csv")
# 提取所有真实-实验网络配对
network_pairs = set()
...
# 创建一个映射：网络名称 -> 最优KS
emd_map = dict(zip(real_net_info['网络名称'].astype(str), real_net_info['RL(KS)']))
...
for i, real_name in enumerate(available_pairs):
    ...
    emd_text = f"KS: {emd_value:.4f}" if emd_value is not None else "KS: N/A"
...
plt.savefig("D:\科研论文文稿\Paper_RL\图\Figs\Fig-DD-KS.pdf", format="pdf")
plt.savefig('D:\科研论文文稿\Paper_RL\图\Figs\Fig-DD-KS.svg', format='svg')
# plt.show()
